...
Illustration of how to run complete ArborMap pipeline either after running step above or not. Start off by running script with help flag. Keep in mind that using the --save (saving KNN object) and --run_UMAP (running and plotting UMAP) flags may significantly increase execution time, depending on the size of your dataset.
```bash
...
```
## Step 6: (Optional) Add Cluster Result to Seurat Object
